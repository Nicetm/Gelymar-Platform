  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (editBtn) {
        ...
        if (!Number.isNaN(id)) startEditAdmin(id);
        return;
      }

      const saveBtn = e.target.closest('.save-admin-edit');
      if (saveBtn) {
        e.preventDefault();
        const id = parseInt(saveBtn.dataset.id, 10);
        const row = saveBtn.closest('tr');
        if (!Number.isNaN(id) && row) saveEditAdmin(id, row);
        return;
      }
      ...
      if (resetBtn) {
        ...
        if (!Number.isNaN(id)) resetAdminPassword(id);
        return;
      }
      ...
      if (deleteBtn) {
        ...
        if (!Number.isNaN(id)) deleteAdmin(id);
        return;
      }

      const removeFormRowBtn = e.target.closest('.remove-admin-form-row');
      if (removeFormRowBtn) {
        e.preventDefault();
        const row = removeFormRowBtn.closest('tr');
        if (row) {
          row.remove();
          updateAdminFormActions();
        }
