...
async function executeWithErrorHandling() {
  try {
    console.log('Llamando al endpoint de procesamiento de items...');
    const response = await axios.post(`${BACKEND_API_URL}/api/cron/check-items`);
    console.log('Procesamiento inicial de items completado:', response.data.message);
  } catch (error) {
    console.error('Error en procesamiento inicial de items:', error.response?.data?.error || error.message);
