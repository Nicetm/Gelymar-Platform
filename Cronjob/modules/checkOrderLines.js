...
async function executeWithErrorHandling() {
  try {
    console.log('Llamando al endpoint de procesamiento de líneas de orden...');
    const response = await axios.post(`${BACKEND_API_URL}/api/cron/check-order-lines`);
    console.log('Procesamiento inicial de líneas de orden completado:', response.data.message);
  } catch (error) {
    console.error('Error en procesamiento inicial de líneas de orden:', error.response?.data?.error || error.message);
