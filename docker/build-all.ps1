...
docker build --no-cache -t nicetm/gelymar-platform:cron -f Dockerfile.cron ..
if ($LASTEXITCODE -eq 0) {
...
Write-Host "   Cron:       docker build --no-cache -t nicetm/gelymar-platform:cron -f Dockerfile.cron .." -ForegroundColor Gray
